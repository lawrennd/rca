...
while ~converged
    ...
    if (options.errorCheck)
        ...
        if (lowerBound_e - lml_new_rca < -1e-6)
            warning([num2str(lowerBound_e - lml_new_rca) ' LowerBound smaller than LML_rca after this E step  !']);
        end
        ...
        if (lowerBound_e - lowerBound_m < -1e-6)  % too high?! was 0
            warning([num2str(lowerBound_e - lowerBound_m) ' drop of LowerBound after this E step !']);
%             break
        end
    end
