...
% walkSkeletons(2:end) = []; walkChannels(2:end) = []; walkXYZChannels(2:end) = []; % remove stealth motions.
...
sigma2_n = trace(Cy)*1e-3;                                   % Noise variance.
...
parfor (i = 1:length(lambda),8)                             % Try different magnitudes of lambda.
    [WWt_hat_new, Lambda_hat_new{i}] = emrca(Y, WWt_hat_old, Lambda_hat_old, sigma2_n, lambda(i), nonZero, emrca_options);
    % Plot results.
