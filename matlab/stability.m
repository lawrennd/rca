...
for iRestart = 1:nRestarts
    ...
    result = struct('Lambda_hat_glasso',Lambda_hat_glasso, ...
                    'GLASSOrocstats',GLASSOrocstats, ...
                    'Lambda_hat_emrca',Lambda_hat_emrca, ...
                    'EMRCArocstats',EMRCArocstats);
...
for iReg = 1:nReg_glasso
    ...
    for iRestart = 1:nRestarts
        ...
        stableLambda_glasso{iReg} = stableLambda_glasso{iReg} + ...
            (abs(triuLambda_hat) > 1e-6);  % Edges in the estimated Lambda from Glasso.
        ...
        stableLambda_idealglasso{iReg} = stableLambda_idealglasso{iReg} + ...
            (abs(triuLambda_hat) > 1e-6);  % Edges in the estimated Lambda from Glasso, under non-confounded conditions.
    end
...
TPs = IDEALGLASSOrocstats(:,1); FPs = IDEALGLASSOrocstats(:,2); FNs = IDEALGLASSOrocstats(:,3);
Recalls = TPs ./ (TPs + FNs);   Precisions = TPs ./ (TPs + FPs);
...
for iReg = 1:nReg_emrca
    ...
    for iRestart = 1:nRestarts
        ...
        stableLambda_emrca{iReg} = stableLambda_emrca{iReg} + ...
            (abs(triuLambda_hat) > 1e-6);  % Edges in the estimated Lambda from EM/RCA.
    end
...
Recalls = TPs ./ (TPs + FNs);   Precisions = TPs ./ (TPs + FPs);
plot(Recalls, Precisions, '-rs'), legend('GL','Ideal GL','EM-RCA');
